    ...
    if (selectedLocation.value == 0) {
        ...
        try {
            ...
            if (status === 200) {
                ...
                const element = document.getElementById("mobileDeliveryDetails");
                if (element) {
        ...
        <div style="padding-bottom: 60px; padding-top: 60px">
            <q-card flat square class="bg-grey-1">
                <q-tab-panels v-if="shoppingCart.length != 0" v-model="tab" swipeable infinite animated
                    ...
                    <q-tab-panel name="checkout" class="fit gradient q-pa-none q-pb-sm">
                        <q-card-section v-for="(item, index) in shoppingCart" :key="item.fg_id" class="q-pa-xs">
                            <q-card class="col">
                                <q-card-section horizontal>
                                    <NuxtImg v-if="item.customFgImgUrl" loading="lazy" ratio="3:4"
                                        placeholder="/placeholder.gif" format="webp" class="col-4" width="200"
                                        quality="50" :src="item.customFgImgUrl" :alt="item.acc_ledger_name"
                                        :title="item.acc_ledger_name" />
                                    <NuxtImg v-else loading="lazy" ratio="3:4" placeholder="/placeholder.gif"
                                        format="webp" class="col-4" width="200" quality="50" :src="item.fg_image"
                                        :alt="item.acc_ledger_name" :title="item.acc_ledger_name" />
                                    <q-checkbox v-model="itemChecked[index]" class="absolute" size="sm"
                                        color="primary" />
                                    <q-card-section class="col no-padding">
                                        <div class="q-pa-sm column q-gutter-y-xs "
                                            @click="navigateTo('/product/' + item.fg_url)">
                                            <div class="text-body1 ellipsis-2-lines text-primary">
                                                <p class="q-ma-none">{{ item.acc_ledger_name }}</p>
                                            </div>
                                            <div class="row justify-between items-center q-my-xs no-wrap">
                                                <div class="row items-center no-wrap">
                                                    <q-icon name="qr_code" class="q-mr-xs" color="primary" />
                                                    <div class="text-caption q-mr-xs ellipsis">
                                                        <p class="q-ma-none">{{ item.fg_sku }}</p>
                                                    </div>
                                                </div>
                                                <div class="text-caption text-grey-6 ellipsis">
                                                    <div class="row no-wrap">
                                                        <q-chip v-if="item.customName" size="12px" style="width: 100px"
                                                            class="ellipsis bg-grey-3 overflow-hidden cursor-pointer no-margin"
                                                            flat square color="primary">
                                                            <q-avatar square>
                                            ...
                                            <div class="row justify-between items-baseline text-primary">
                                                <div v-if="item.fg_price != item.fg_discounted_price"
                                                    class="text-caption text-strike">
                                        ...
                                        <div class="q-pa-sm q-col-gutter-y-xs">
                                            <div class="row justify-center items-center q-gutter-y-sm">
                                                ...
                                                <div class="row justify-between items-center q-gutter-x-md q-pl-sm">
                                                    <q-btn outline round icon="remove" color="primary" size="xs"
                                                        aria-label="Remove" @click="removeQty(index)" />
                                                    <q-input v-model.number="item.cartQty" dense outlined type="number"
                                                        color="primary" style="max-width: 40px" min="1" readonly />
                                                    <q-btn outline round icon="add" color="primary"
                                                        :disable="disabledAddQtyBtn" size="xs" aria-label="Add"
                                                        @click="addQty(index)" />
                                                    <q-btn icon="delete_outline" outline round size="sm"
                                                        class="bg-white" color="primary" aria-label="Delete"
                                                        @click="deleteProductModal(index)" />
                                                </div>
                                            </div>
                                        </div>
                                        <!-- Delivery Type -->
                                        <div v-show="selectedLocation != 0 && regDevFee[index] > 0"
                                            class="items-center q-px-sm q-py-xs">
                                        ...
                                        <q-separator v-show="exprDevFee[index] > 0 && item.pre_order_qty == 0" />
                                        <div v-show="exprDevFee[index] > 0 && item.pre_order_qty == 0"
                                            class="items-center q-px-sm q-py-xs">
                                        ...
                                        <q-separator v-show="selectedLocation != 0 && item.fg_shipping_cost_free == 1
                                            " />
                                        <!-- Free Shipping -->
                                        ...
                                        <div v-if="item.pre_order_qty > 0" class="row justify-center q-pb-xs text-caption text-weight-medium
                                             text-primary">
                ...
                <q-card v-else flat bordered class="row justify-center items-center q-mx-xs gradient"
                    style="height: 120px">
                    <q-card-section class="text-uppercase text-h6 q-py-xl text-weight-regular text-center">
                        <h6 class="q-ma-none"> {{ noItemText }}</h6>
                    </q-card-section>
                </q-card>
                <!-- Delete Modal -->
            ...
            <!-- Delivery Details -->
            <div v-if="shoppingCart.length != 0" id="mobileDeliveryDetails" class="q-gutter-y-sm q-pt-sm">
                <q-card flat square class="q-pa-xs text-center">
                    <q-card-section class="q-gutter-y-sm q-pa-sm">
                        <q-input filled v-model="customerName" type="text" label="Name"
                            placeholder="Please enter your name" dense :rules="[(val) => !!val || 'Field is required']"
                            @update:model-value="checkDisabledBtn" required>
                        ...
                        <q-input filled v-model="customerPhone" placeholder="017XXXXXXXX" hint="017XXXXXXXX" dense
                            maxlength="11" type="tel" label="Mobile Number*" required
                            :rules="[(val) => !!val || 'Field is required']" @update:model-value="checkDisabledBtn">
                            <template v-slot:prepend>
                                <q-icon size="xs" name="phone" />
                            </template>
                        </q-input>
                        <small v-if="phoneError" class="text-primary text-capitalize q-pl-sm">{{ phoneError }}</small>
                        <q-input filled v-model="customerEmail" type="email" label="Email"
                            placeholder="example@mail.com" dense>
                            <template v-slot:prepend>
                    ...
                    <q-card v-if="shoppingCart.length != 0" id="total" class="gradient rounded-borders">
                        ...
                        <q-card-section class="row">
                            ...
                            <q-input v-model:model-value="coupon_code" filled :hint="coupon_code
                                ...
                                <template #after>
                                    <q-btn type="submit" style="width: 120px; height: 40px" color="primary"
                                        @click="addCoupon()">
                                        Add
                                    </q-btn>
                                </template>
                        ...
                        <q-card-section class="q-py-none">
                            <div class="column q-gutter-y-sm">
                                <div v-if="couponDiscountAmount > 0"
                                    class="col text-subtitle1 column text-weight-medium">
                                    <h6 class="text-bold text-primary text-h6 q-pb-xs q-ma-none">
                                        Invoice Total {{ config.public.currencyBefore }}
                                    </h6>
                                    <h6 class="text-primary text-h6 text-weight-medium q-ma-none">
                                ...
                                <div v-else class="col column text-weight-medium">
                                    <h6 class="text-bold text-h6 q-pb-xs q-ma-none text-primary">
                                        Invoice Total {{ config.public.currencyBefore }}
                                    </h6>
                                ...
                                <div class="column text-weight-medium">
                                    <h6 class="text-h5 text-primary q-pb-sm q-ma-none">
                                        Payable Amount {{ config.public.currencyBefore }}
                                    </h6>
                                    <h5 class="text-primary text-h4 text-bold q-ma-none">
                                        {{
                                            formatMoney(
                                                cartTotal * 1.0 -
                        ...
                        <q-card-section class="column items-center justify-center q-gutter-y-md">
                            <q-form :action="getEnvValue.api_url + 'paynow-order'" id="payNowForm" method="post"
                                class="column q-gutter-md">
                                <div v-for="(payment, index) in paymentGatewayList" :key="payment.payment_gateway_id">
                                    <q-input v-model="getEnvValue.c_code" style="display: none" name="company_code" />
                                    <q-input v-model="paymentGatewayID" style="display: none"
                                    ...
                                    <div v-for="(cartItems, index2) in shoppingCart" :key="cartItems">
                                        <q-input v-model="shoppingCart[index2].fg_id" style="display: none"
                                            name="cart_items_fg_id[]" />
                                        <q-input v-model="shoppingCart[index2].item_checked" style="display: none"
                                            name="cart_items_item_checked[]" />
                                        <q-input v-model="shoppingCart[index2].fg_discounted_price"
                                            style="display: none" name="cart_items_fg_discounted_price[]" />
                                        <q-input v-model="shoppingCart[index2].cartQty" style="display: none"
                                            name="cart_items_cartQty[]" />
                                        <q-input v-model="shoppingCart[index2].delivery_fee" style="display: none"
                                            name="cart_items_delivery_fee[]" />
                                        <q-input v-model="shoppingCart[index2].delivery" style="display: none"
                                            name="cart_items_delivery[]" />
                                        <q-input v-model="shoppingCart[index2].delivery_day" style="display: none"
                                            name="cart_items_delivery_day[]" />
                                        <q-input v-model="shoppingCart[index2].selectedAttr" style="display: none"
                                            name="cart_items_selectedAttr[]" />
                                        <q-input v-model="shoppingCart[index2].fg_price" style="display: none"
                                            name="cart_items_fg_price[]" />
                                        <q-input v-model="shoppingCart[index2].customID" style="display: none"
                                            name="cart_items_custom_id[]" />
                                        <q-input v-model="shoppingCart[index2].customName" style="display: none"
                                            name="cart_items_custom_name[]" />
                                        <q-input v-model="shoppingCart[index2].customPrice" style="display: none"
                                            name="cart_items_custom_price[]" />
                                    </div>
                                    ...
                                    <q-radio v-model="paymentSelect" :val="index"
                                        @click="selectPayment(payment.payment_gateway_id)"
                                        :title="disabledBtn ? 'Fill Up The Delivery Details' : ''"
                                        :disable="disabledBtn">
                                        <q-btn ref="submit" type="button" outline color="primary"
                                            :label="payment.payment_gateway_name" no-caps class="bg-grey-1 shadow-3"
                                            style="min-width: 160px">
                                        </q-btn>
                                    </q-radio>
                                </div>
                                <q-radio v-model="paymentSelect" :val="-1"
                                    :title="disabledBtn ? 'Fill Up The Delivery Details' : ''" :disable="disabledBtn">
                                    <q-btn outline color="primary" style="min-width: 160px" class="bg-grey-1 shadow-3"
                                        no-caps>
                                        Cash On Delivery
                                    </q-btn>
                                </q-radio>
                                <q-btn @click="payAndOrder()" style="min-width: 160px" v-ripple label="Pay Now"
                                    color="primary" class="q-ml-md"
                                    :title="disabledBtn ? 'Fill Up The Delivery Details' : ''"
                ...
                <q-dialog v-model="deliveryLocationModal" backdrop-filter="blur(1px) brightness(90%)"
                    transition-show="fade" transition-hide="fade">
                    <q-card flat bordered style="width: 400px">
                        <q-card-section class="q-py-sm">
                            <div class="text-h6 text-primary text-capitalize text-center">
                                Delivery Location
                            </div>
                        </q-card-section>
                        <q-card-section class="q-py-sm">
                            <q-input ref="filterRef" v-model="filter" class="bg-white" dense outlined
                            ...
                            <q-scroll-area style="height: 400px" class="q-pa-sm">
                                <q-tree class="text-capitalize text-body2" default-expand-all dense
                                    :nodes="locationJson" :filter="filter" node-key="label" label-key="label"
                                    color="primary">
                                    <template #header-select="prop">
                                        <q-radio v-model="selected" v-close-popup size="xs" :val="prop.node.chain"
                                            :label="prop.node.label" @click="
                                                locationChanged(
                                                    prop.node.id,
            ...
            <q-page-sticky expand position="top">
                <q-toolbar class="text-primary bg-grey-2 shadow-2 q-py-sm q-px-sm">
                    <q-toolbar-title class="text-body1 text-weight-medium">
                        <h6 class="q-ma-xs text-h6 text-bold">Checkout</h6>
                    </q-toolbar-title>
                    <q-btn v-if="checkedAll && shoppingCart.length != 0" v-model="checkedAll" icon="delete_outline" flat
