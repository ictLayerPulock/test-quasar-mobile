  ...
  <div class="bg-grey-2">
      ...
      <q-carousel ref="carousel" v-model="slide" swipeable :autoplay="60000" animated height="240px">
        <q-carousel-slide v-for="(item, index) in addImgUrl" :key="item" class="no-padding" :name="index + 1">
          <div style="width:100%; height:230px">
            <NuxtImg loading="lazy" width="356" height="230" format="webp" class="cursor-pointer fit" quality="50" :draggable="false" placeholder="/placeholder.gif" :src="blog.add_img_url[index]" :alt="blog.blog_title" :title="blog.blog_title" />
          </div>
        </q-carousel-slide>
        <q-carousel-slide v-if="blog.blog_video" class="no-padding" :name="addImgUrl.length + 1" label="Next" style="height:230px; width: 100%;">
          ...
          <ScriptYouTubePlayer ref="video" :ratio="16 / 9" class="no-padding" :video-id="blog.blog_video" style="height: 100%; width: 100%">
    ...
    <q-card flat>
              ...
              <q-icon size="sm" name="share" color="negative" class="q-mr-xs">
                <q-menu class="bg-white q-pa-none" auto-close>
                  <q-list class="q-pa-none">
                    <q-item clickable>
                      <q-btn dense href="#" target="_blank" size="sm" flat comfortable class="rounded-borders text-red" icon="email" title="Share Email" />
                    </q-item>
                    <q-separator />
                    <q-item clickable>
                      <q-btn href="#" dense target="_blank" size="sm" flat comfortable color="white" class="rounded-borders text-blue-6" icon="facebook" title="Share Facebook" />
                    </q-item>
                    <q-separator />
                    <q-item clickable>
                      <q-btn dense href="https://wa.me/YOUR_NUMBER" target="_blank" size="sm" flat comfortable class="rounded-borders" icon="language" title="Share Affiliate"></q-btn>
                    </q-item>
                  </q-list>
                </q-menu>
              </q-icon>
      ...
      <q-card-section class="q-pa-sm">
        <h1 class="text-h6 q-my-none text-primary text-weight-medium text-capitalize text-center">
          {{ blog.blog_title }}
        </h1>
      </q-card-section>
    </q-card>
