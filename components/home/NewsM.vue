      ...
      <q-scroll-area style="height: 273px" :thumb-style="{ opacity: '0' }" @touchstart.stop @mousedown.stop>
        <q-card-section v-if="status === 'pending'" class="row q-pa-sm q-col-gutter-sm" style="width: 890px">
          <div v-for="item in 6" :key="item" class="col-2">
            <q-card class="shadow-5 bg-grey-1">
              <NuxtImg loading="lazy" sizes="100vw sm:50vw md:170px" width="170"  quality="50" class="fit"   format="webp" src="/placeholder.gif"
                  :draggable="false" />
                  <q-card-section class="q-pa-sm">
                <div class="text-body2 text-secondary text-weight-regular ellipsis-2-lines text-capitalize" style="height: 42px">
        ...
        <q-card-section v-else class="row q-px-sm q-pt-sm q-pb-none q-col-gutter-sm" style="width: 890px">
          <div v-for="item in response.data" :key="item">
            <NuxtLink :to="`/news/${item.news_url}`" :aria-label="item.news_title" style="text-decoration: none">
              <q-card class="shadow-5 overflow-hidden" style="width:165px">
                <q-card-section class="q-pa-none border-bottom row">
                  <NuxtImg loading="lazy" width="165" height="165" format="webp" quality="50" :draggable="false" placeholder="/placeholder.gif" :src="item.news_image" :alt="item.news_title" :title="item.news_title" />
                </q-card-section>
                <q-card-section class="q-pa-sm">
                ...
                <q-card-section class="q-pa-sm">
                  <div class="items-center justify-between row">
                    ...
                    <div v-if="item.news_view > 0" class="items-center row text-caption">
