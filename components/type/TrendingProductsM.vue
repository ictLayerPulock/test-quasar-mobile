 ...
 <section v-show="show">
    <q-card flat square class="gradient">
      ...
      <q-scroll-area style="height:308px" :thumb-style="{ opacity: '0' }" @touchstart.stop @mousedown.stop>
        <q-card-section v-if="status === 'pending'" class="row q-pa-sm q-col-gutter-sm gradient" style="width: 1000px">
...
const { data: response, status }: ProductType = useAsyncData(
  `type-trending-product-by-url: ${props.url}`,
  () =>
    $fetch("/api/type-trending-product-by-url/" + props.url, {
      query: {
        fg_type_url: props.url,
        limit: "12",
        start: "0",
      },
    }),
